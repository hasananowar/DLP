...
class Data_FILT_HB(data.Dataset):
    ...
    def __getitem__(self, index):
        # Ensure float64 keys are cast to int before lookup
        src_node = int(self.df.iloc[index].src)
        dst_node = int(self.df.iloc[index].dst)
        idx1 = self.idx_map[src_node]
        idx2 = self.idx_map[dst_node]
...
def load_data_link_prediction_FILT_HB(path, inp):
    print('Loading FILT_HB dataset...')
    path_up = path[:-5]
    df_data = pd.read_csv(path + '/train.csv')    
    df_drug_list = pd.read_csv(path_up + '/unique_nodes.csv')

    idx = df_drug_list['src'].tolist()
    idx = np.array(idx)
    idx_map = {j: i for i, j in enumerate(idx)}
    
    df_data_t = df_data[df_data.label == 1]
    edges_unordered = df_data_t[['src', 'dst']].values    
    
    if inp == 'node2vec':
        emb = pd.read_csv(path + 'FILT_HB.emb', skiprows=1, header = None, sep= ' ').sort_values(by = [0]).set_index([0])
        for i in np.setdiff1d(np.arange(len(df_drug_list)), emb.index.values):
            emb.loc[i] = (np.sum(emb.values, axis = 0)/emb.values.shape[0])
        features = emb.sort_index().values
    elif inp == 'one_hot':
        features = np.eye(len(df_drug_list))
