...
from evaluate_models_utils import evaluate_model_link_prediction, modified_evaluate_model_link_prediction
    ...
    for run in range(args.num_runs):
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                pos_edge_label = torch.tensor(batch_labels, dtype=torch.float, device=predicts.device)
                labels = torch.cat((pos_edge_label, torch.zeros_like(pos_edge_label)), dim=0)
                labels = labels.to(predicts.device)
            ...
            val_losses, val_metrics, val_preds, val_labels = modified_evaluate_model_link_prediction(model_name=args.model_name,
                                                                     model=model,
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                test_losses, test_metrics, test_preds, test_labels = modified_evaluate_model_link_prediction(model_name=args.model_name,
                                                                           model=model,
                                                                           neighbor_sampler=full_neighbor_sampler,
        ...
        logger.info(f'Training & Validation {run + 1} cost {train_val_time:.2f} seconds.')

        temp_result_dir = "./temp_result"
        os.makedirs(temp_result_dir, exist_ok=True)

        # # Save runtime to file
        # runtime_file = os.path.join(temp_result_dir, f"runtime_{args.dataset_name}_{args.save_model_name}.txt")
        # with open(runtime_file, "w") as f:
        #     f.write(f"cost {train_val_time:.2f} seconds.\n")
        ...
        output_file = os.path.join(temp_result_dir, f"outputs_{args.dataset_name}_{args.save_model_name}.pt")
        torch.save({"test_preds": test_preds, "test_labels": test_labels}, output_file)
        ...
        test_time = time.time() - infer_start_time
        logger.info(f'test time: {test_time:.4f} seconds.')

        logger.info(f'test loss: {np.mean(test_losses):.4f}')
        

        # Save train+val time and test time into a single JSON
        time_json = {
            "dataset": args.dataset_name,
            "model": args.save_model_name,
            "train_validation_time_seconds": round(train_val_time, 4),
            "test_inference_time_seconds": round(test_time, 4),
            "Trainable params": get_parameter_sizes(model),
            "Trainable params (MB)": (get_parameter_sizes(model)*4)/1024/1024
        }

        save_time_file = os.path.join(
            temp_result_dir,
            f"summary_{args.dataset_name}_{args.save_model_name}.json"
        )

        with open(save_time_file, "w") as f:
            json.dump(time_json, f, indent=4)

        logger.info(f"Saved time summary to {save_time_file}")
        #Hasan
        # for metric_name in test_metrics[0].keys():
        ...
        # save_result_folder = f"./saved_results_original/{args.model_name}/{args.dataset_name}"
